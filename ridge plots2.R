...
# overall also includes life_historyperennial AND its interactions
life_history_ints <- c(
  "stemp_sc:life_historyperennial",
  "sprecip_sc:life_historyperennial",
  "latitude_sc:life_historyperennial",
  "elevation_sc:life_historyperennial"
)

overall_terms  <- c(species_main, "life_historyperennial", species_int, life_history_ints)
...
relabel <- function(t){
  if (str_detect(t,":")){
    ...
    # Handle life_history interactions separately
    if (any(parts == "life_historyperennial")) {
      # Find the non-life_history part
      other_part <- parts[parts != "life_historyperennial"]
      return(paste0(labmap[other_part], " × Life history"))
    } else {
      # Regular climate × climate interactions
...
# factor levels: mains first, then climate×climate interactions, then life history interactions
...
life_int_labs <- c(
  "Temperature × Life history",
  "Precipitation × Life history",
  "Latitude × Life history",
